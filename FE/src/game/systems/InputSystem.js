 ...
 *   ★ 이번 값은 "몇 px 내린다"가 아니라 **엄지 도달 범위에서 유도**한 값이다.
 *
 ...
 *           → **y ∈ [198, 252]**
 *       두 추정(212.5 / 225)은 전부 이 구간 안이다. 그 중 **아래쪽에 덜 붙은 212.5** 를 택하고
 *       바닥 여백이 읽기 쉬운 150 이 되도록 **210** 으로 내림했다.
 *       ★ 아래쪽에 덜 붙인 이유: 0차 값 (120, 200) 이 "너무 좌측 하단"이라는 평가를 받았다.
 *         다만 그 원인은 y 가 아니라 x 였다 — x=120 에서 중심선은 y ≈ 233 이라 200 은
 *         오히려 중심선보다 33 **위**였다. 사용자가 이번에 x=145 를 "좋다"고 못박은 것과도 맞는다.
 *         그래도 그 제보가 유일한 아래쪽 실측치이므로 중심선보다 살짝 위로만 간다.
 *       ★ 1차 값 165 는 아크 상단(150)에 15 밖에 안 떨어져 있었다. 엄지를 **최대로 폈을 때만**
 *         닿는 자리라 "더 아래로"라는 제보와 정확히 맞물린다.
 *
 *   [3] 하한·상한 검사 (전부 헤드리스 실측으로 확인했다)
 *       · 링 아래끝 210 + 48 = 258 ≤ 360 — 화면 바닥까지 **102** 남는다
 *       · 링 위끝 210 − 48 = 162 — HP 바(y 12~22) 에서 140, HP 숫자(y 26) 에서 136,
 *         인간성 심장(y 38~62, 실측값) 에서 **100** 여유. 겹침 0
 *         (덤으로 PACT 전용인 녹턴 초상 y 66~131 과도 31 떨어져 있다)
 *       · 링 전체 162~258 이 아크 150~300 **안에 통째로** 들어간다 (위 12 / 아래 42 여유).
 *         165 일 때는 링 위끝 117 이 아크 밖으로 33 나가 있었다 — 이번 값이 명확히 낫다
 *       · x 는 손대지 않았다. 링 우끝 193 < 중앙 시야 금지선 200 이 그대로 유효하다
 *
 *   [4] 잡기 반경 80 이 화면 밖으로 나가는가 → **안 나간다 (0 px)**
 *       잡기 원은 x 65~225 / y 130~290 이다. 좌측은 65 남고(레터박스 확장까지 있으니 더 여유),
 *       아래는 360 − 290 = **70** 남는다. 즉 실효 잡기 면적이 잘리지 않아 반경 80 이 온전히 쓰인다.
 *       ★ 이게 y 를 252 까지 더 내리지 않은 이유이기도 하다 — y > 280 이면 잡기 원이
 *         화면 바닥을 넘어 아래쪽 절반이 눌리지 않는 원이 된다(보이는 곳 ≠ 먹히는 곳).
 *       · 대시 버튼(우측에서 84 / 히트 72×72 → 640 폭에서 중심 556,292)까지 중심 거리
 *         hypot(556−145, 292−210) = 419. 반경합 80 + 36 = 116 이라 여유 303,
...
const FIXED_JOY_MARGIN_B = 150; // y = h - 150 -> 360 화면에서 210
