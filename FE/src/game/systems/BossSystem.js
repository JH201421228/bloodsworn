...
export class BossSystem {
    ...
    spawn() {
        ...
        const d = this.def;
        // 세로 360 고정 / 가로 640~864 가변 (config.js 좌표계 주석) — 좌우에서 들어와야
        // 보스 전신이 화면에 들어온다. 위아래에서 오면 등장 순간 프레임의 절반이 화면 밖이다.
        // ★ 화면이 넓어져도 이 전제는 더 안전해질 뿐이다. 등장 거리 210 은 어느 비율에서도
        //   가로 반폭(320~432) 안이라 보스가 화면 안에 들어온 채로 등장한다.
     ...
     * ★ 1세대만 분열한다. 무한 분열은 가장 좁은 화면(640x360)을 몇 초 만에 채운다.
     *   넓은 화면(최대 864x360)이라고 완화하지 않는다 — 분열은 화면 면적이 아니라
     *   플레이어 주변의 회피 공간을 지우는 패턴이라 폭이 늘어도 위험도가 줄지 않는다.
     */
    splitBolt(s) {
        if (!s.splitN) return;
