...
## 12. ★ §9(궤 발밑 표식) 수령 결과 — 시트가 아니라 낱장으로 왔고 그대로 배선했다 (2026-08-13)

### 12.1 시트 A 를 다시 굽지 않았다

§9 는 *"시트 A 예비 칸 05 에 넣는다"* 고 적었다. 그런데 수령본은 **`mark-chest-48.png`
낱장 한 장**(48x48)이었다. 이쪽이 오히려 맞다 — §9 자신이 *"`enc-decal-48.png` 는 이미
수령했다. **절대 덮어쓰지 않는다**"* 고 못 박았고, 칸 하나를 채우자고 6칸 시트를 통째로
다시 받으면 이미 검수를 통과한 칸 00~04 를 잃는다.

그래서 **별도 텍스처 키 `mark-chest`** 로 등록했다. 낱장이라 텍스처 유닛을 하나 더 쓰지만,
궤와 좌판은 같은 화면에 안 뜬다 — §3.5 가 시트를 둘로 나눈 것과 같은 근거다.

| | §9 의 계획 | 실제 |
|---|---|---|
| 파일 | `enc-decal-48.png` 칸 05 | `FE/public/assets/decal/mark-chest-48.png` (낱장 48x48) |
| 키 | `enc-decal-48` 프레임 5 | `mark-chest` (`images[]` 한 줄) |
| 폴백 판정 | `hasPedTex` | 자기 키로 `textures.exists("mark-chest")` |

### 12.2 수령 검수 — 통과. 다만 알파 사다리가 없다

| 항목 | 결과 |
|---|---|
| 치수 · 채널 | 48x48 · 알파 있음 ✅ |
| 네 모서리 알파 | 0 / 0 / 0 / 0 ✅ |
| **§5.1 무채색** | **유채색 화소 0** ✅ — 두 계조 `#4E4E4E` · `#9D9D9D` 뿐 |
| 여백(V-4) | 그림 34x32 · L7 R7 T8 B8 — **좌우·상하 대칭** ✅ |
| 중앙 비움 | 안쪽이 비어 있다 ✅ (궤 32x32 가 그 위에 선다) |
| **§5.2 알파 사다리** | **없다.** 불투명 화소 501개가 전부 alpha 255 다 ❌ |

★ 알파 사다리는 **코드가 준다** — `EncounterSystem` 이 `setAlpha(0.62)` 로 얹는다.
§5.2 가 금지한 것은 "바닥 전체가 불투명해서 그 위를 지나는 적과 투사체가 안 보이는 것"이고,
이 그림은 **가운데가 비어 있는 테두리 판**이라 가리는 면적이 원래 없다. 그림을 고쳐 굽는 것보다
코드 상수 하나가 낫다고 판단했다 — 수령본은 손대지 않는다는 원칙과도 맞는다.

> 다음 의뢰에 반영할 것: §5.2 표를 **"알파를 그림에 굽는다"**로 요구할지
> **"알파는 코드가 준다"**로 요구할지 한쪽으로 정한다. 지금 두 시트(A·B)는 굽고 이 낱장은
> 안 굽는다 — 규격이 두 벌이면 다음 사람이 반드시 틀린다.

### 12.3 배선 — §9 의 절차 그대로, 지우는 자리만 하나 정했다

```
EncounterSystem.buildObjects()  this.chestMark = textures.exists("mark-chest") ? add.image(...) : null
EncounterSystem.beginChest()    표식 위치를 잡고 보이게 한다
EncounterSystem.rollChestReward() 궤가 열리는 순간 표식을 걷는다   ← §9 가 안 정한 자리
EncounterSystem.clear()         런이 끝나면 함께 내린다
```

★ **`hideAll()` 에 넣지 않았다.** 궤는 조우가 닫혀도 그 자리에 그대로 남는다(`30` §4.4).
표식이 조우와 함께 사라지면 궤만 덩그러니 남아 §9 가 지적한 원래 문제로 되돌아간다.
그래서 지우는 자리는 **궤가 열리는 순간** 하나뿐이다. `rollChestReward` 는 필드보스 보상
궤도 부르므로 좌표로 같은 자리인지 확인한다(반경 16px = 궤 스프라이트의 절반).

★ **회전 없음 · tint 없음.** §9 가 정한 그대로다. 궤는 맥동만 한다.

★ **원으로 대신하지 않는다.** 텍스처가 없으면 `chestMark` 는 `null` 이고 아무것도 안 그린다 —
§4.1 의 03·04 와 같은 규약이다. `assets.json` 에서 `mark-chest` 한 줄을 빼면
2026-08-12 이전의 "궤만 덩그러니" 상태로 정확히 되돌아간다.
