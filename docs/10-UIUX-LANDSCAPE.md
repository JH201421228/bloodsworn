...
| 조이스틱 베이스 | **(120, 200)** 고정 / 플로팅은 동적 | r=40 (링 2px) | 고정 원점 (5.3) | 대기 **0.22** / 터치 중 0.85 | 반경 80 | 기본은 고정. 좌표 근거는 5.3 |
...
세로 360 = 헤더(48) + 구분선(2) + 여백(2) + [카드240] + 여백(8)
           + [버튼40] + 하단여백(20)
         = 48 + 2 + 2 + 240 + 8 + 40 + 20 = 360  ✔ 정확히 맞음
           (카드 y=52~292 / 버튼 y=300~340)
```
...
3. **플레이어 캐릭터는 항상 화면 중앙(320,180)** — 조이스틱(고정 원점 (120,200))과 200px 떨어져 있어 손가락과 절대 겹치지 않는다. 링 오른쪽 끝(x=168)과 중앙 시야 금지 구역(x≥200) 사이에도 32px 여유가 있다.

### 5.3 고정 모드 (★ 기본값)

| 항목 | 값 |
|---|---|
| 위치 | 중심 **(120, 200)** 고정 — x 는 좌측 절대값, y 는 **바닥에서 160** |
...
| 조건 | 내용 | 판정 |
|---|---|---|
| 왼손 엄지 아크 | 문서에 있는 스윕 아크 `x≈420~600 / y≈150~300` 은 **오른손** 기준이다(2.2). 왼손 데이터가 없어 640 폭 기준으로 좌우 반전해 `x 40~220 / y 150~300` 을 근거로 삼는다. 양손 파지에서 왼손은 화면 **왼쪽 끝**을 잡으므로 x 는 좌측 절대값이고, 논리 폭이 640~864 로 늘어도 변하지 않는다 | (120, 200) ∈ 아크 |
| 반경 48 이 화면 안 | x−48 ≥ 0, 48 ≤ y ≤ 312 | 72~168 / 152~248 ✅ |
| 중앙 시야 금지 구역 | x ≥ 200 침범 금지(2.4) → x + 48 ≤ 200 → x ≤ 152 | 168 ✅ |
| 좌상단 HUD 회피 | EXP 바 y 0~4 / HP 바 y 12~22 / HP 숫자 y 26 / 인간성 심장 y 38~62 (x 16~120) / 녹턴 초상 y 66~131 (x 6~58) | 링 위끝 152 — 최하단 요소와 21px 여유 ✅ |
| 대시 버튼(우하단) | 우측에서 84, 히트 72×72 → 640 화면에서 x 520~592 | 화면 반대편, 겹침 0 ✅ |

→ 조건 교집합은 `x ∈ [106, 152] / y ∈ [179, 300]` 이고 그 안에서 **우·상단 끝쪽**을 잡은 값이 (120, 200) 이다.
좌하단 옛 위치 (84, 284) 와는 91px 떨어져 있어 활성 반경 80 밖이다 — 옛 자리를 눌러도 안 먹는다(= 보이는 곳과 먹히는 곳이 같다).
