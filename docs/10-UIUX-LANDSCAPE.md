...
| 조이스틱 베이스 | 동적 | r=40 (링 2px) | 터치 지점 | **0.30** | — | 정본 §3.3 반투명 30% |
...
| 헤더 구분선 | 0, 50 | 640×2 | — | `--blood` 그라데이션 |
| **카드 1** | **28, 60** | **184×224** | — | |
| **카드 2** | **228, 60** | **184×224** | — | 좌우 gap 16 |
| **카드 3** | **428, 60** | **184×224** | — | 우측 여백 28 |
| 리롤 버튼 | 148, 300 | **156×40** (시각) | — | 히트 **176×54**. 잔여 0회면 `--ash` 비활성 |
| 거절(스킵) 버튼 | 336, 300 | **156×40** (시각) | — | 히트 **176×54** |
| 하단 여백 | — | 20 | — | 제스처 내비 바 회피 |

**가로 3분할 계산 (왜 이 숫자인가)**

```
640 = 좌여백(28) + [카드184] + gap(16) + [카드184] + gap(16) + [카드184] + 우여백(28)
    = 28 + 184 + 16 + 184 + 16 + 184 + 28 = 640  ✔ 정확히 맞음
```

- 카드는 **세로로 긴 직사각형(184×224, 비율 1:1.22)**. 가로 화면에서 3장을 나란히 놓으면 각 카드는 필연적으로 세로형이 되고, 이게 오히려 "계약서/증서" 모티프(정본 §7 톤 가이드)와 정확히 맞는다.
- 세로형 카드는 **정보를 위→아래로 쌓을 수 있다** = 축복(위) / 구분선 / 대가(아래)라는 서사 순서가 그대로 시각 순서가 된다.

---

#### 2.5.1 카드 1장 내부 레이아웃 (184 × 224)

```
     ┌── x: 0 ────────────────────────── 184 ──┐
 y=0 ╔══════════════════════════════════════════╗  ← 등급 테두리 3px (Epic은 4px + 외곽 글로우)
     ║ ◆ EPIC                            [아이콘]║  y  4 ~ 24   등급 배지 + 축복 아이콘 24×24
  28 ╟──────────────────────────────────────────╢
     ║                                          ║
     ║   폭 군 의   손 톱                        ║  y 34 ~ 52   ★ 축복 이름 16px (최대 크기)
     ║                                          ║
     ║   공격력 +22%                            ║  y 58 ~ 70   축복 효과 12px
     ║                                          ║
  84 ╟────────────  ✖ 대 가  ────────────────────╢  y 84 ~ 96   구분선 + 인장 (붉은 1px + 중앙 라벨)
     ║                                          ║
     ║  [ ◒ 갈  증 ]                            ║  y 102 ~ 118 대가 태그 칩 14px (도형+색+이름)
     ║                                          ║
     ║  초당 HP -1.1        ×2중첩              ║  y 124 ~ 136 대가 수치 12px  `--blood-ink`
     ║                                          ║
     ║  중첩  ● ● ○                              ║  y 142 ~ 156 중첩 인디케이터 (원 10px, gap 6)
     ║                                          ║
     ║  ✦ 이 계약으로 각성한다                   ║  y 162 ~ 176 각성 임박 문구 11px `--gold` (조건부)
 184 ╟──────────────────────────────────────────╢
     ║  인간성  −12       ♥♥♥♡♡ ▸ ♥♥♡♡♡         ║  y 190 ~ 206 인간성 비용 11px + 변화 미리보기
 224 ╚══════════════════════════════════════════╝
```

| # | 요소 | x(카드 상대), y | 크기 | 폰트 | 색 | 정렬 |
|---|---|---|---|---|---|---|
| 1 | 등급 테두리 | 0, 0 | 184×224 | — | 등급색 | Common 2px / Rare 3px / Epic 4px+글로우 |
| 2 | 등급 배지 (도형+글자) | 8, 6 | 자동 | 10px | 등급색 | L/T. `○ COMMON` `◇ RARE` `◆ EPIC` |
| 3 | 축복 아이콘 | 152, 4 | 24×24 | — | — | R/T. Raven Fantasy 16×16을 1.5배 |
| 4 | **축복 이름** | 12, 34 | 160×18 | **16px** | `--parchment` | L/T. **화면 내 최대 문자** |
| 5 | 축복 레벨 표기 | 12, 54 | 자동 | 11px | `--candle` | 무기일 때만. `Lv.2 ▸ Lv.3` |
| 6 | 축복 효과 | 12, 58(또는 70) | 160 폭 | 12px | `--candle` | L/T. 최대 2줄 |
| 7 | 구분선 + `✖ 대가` 라벨 | 8, 84 | 168×12 | 10px | `--blood` | C. 선 1px + 라벨 배경 파냄 |
| 8 | 대가 태그 칩 | 12, 102 | 자동×16 | **14px** | 태그색 + 도형 | L/T. `[◒ 갈증]` — **도형·아이콘 병용** |
| 9 | 대가 수치 | 12, 124 | 160 폭 | 12px | `--blood-ink` | L/T |
| 10 | Epic 2중첩 배지 | 152, 124 | 자동 | 10px | `--blood-ink` | R/T. `×2` |
| 11 | **중첩 인디케이터** | 12, 142 | 자동×10 | — | 채움/빈 원 | L/T. `● ● ○` 원 지름 10, gap 6 |
| 12 | 각성 임박 문구 | 12, 162 | 160 폭 | 11px | `--gold` | L/T. **조건부 표시** |
| 13 | 하단 구분선 | 8, 184 | 168×1 | — | `--stone-line` | |
| 14 | 인간성 비용 | 12, 190 | 자동 | 11px | `--blood` | L/T |
| 15 | 인간성 변화 미리보기 | 172, 190 | 자동 | 11px | `--ash`→`--blood` | R/T |
...
3. **플레이어 캐릭터는 항상 화면 중앙(320,180)** — 조이스틱(좌하단)과 최소 150px 떨어져 있어 손가락과 절대 겹치지 않는다. 이게 플로팅 조이스틱을 쓰는 가장 큰 이유다.
