...
| 조이스틱 베이스 | **(145, 210)** 고정 / 플로팅은 동적 | r=48 (링 2px) | 고정 원점 (5.3) | 대기 **0.22** / 터치 중 0.85 | 반경 80 | 기본은 고정. 좌표 근거는 5.3 (2026-08-12 재조정) |
...
3. **플레이어 캐릭터는 항상 화면 중앙(320,180)** — 조이스틱(고정 원점 (145,210))과 hypot(175,30)=178px 떨어져 있어 손가락과 절대 겹치지 않는다. 링 오른쪽 끝(x=193)과 중앙 시야 금지 구역(x≥200) 사이에도 7px 여유가 있다.

### 5.3 고정 모드 (★ 기본값)

| 항목 | 값 |
|---|---|
| 위치 | 중심 **(145, 210)** 고정 — x 는 좌측 절대값, y 는 **바닥에서 150** |
...
**원점 (145, 210) 의 근거 — 「가로 파지에서 왼손 엄지가 편한 자리」**

> 2026-08-12 실기기 제보로 두 번 옮겼다. (120, 200) → (145, 165) → **(145, 210)**.
> 마지막 값은 "몇 px 내린다"가 아니라 **엄지 도달 범위에서 유도**했다.
...
두 추정(212.5 / 225)이 모두 `[198, 252]` 안이다. 아래쪽에 덜 붙은 **212.5** 를 택하고
바닥 여백이 읽기 쉬운 150 이 되도록 **210** 으로 내림했다.

- 아래쪽에 덜 붙인 이유: 0차 값 (120, 200) 이 "너무 좌측 하단"이라는 평가를 받았다. 다만 원인은 y 가 아니라 **x** 다 — x=120 에서 중심선은 y ≈ 233 이라 200 은 오히려 중심선보다 33 **위**였다. 사용자가 이번에 x=145 를 "좋다"고 못박은 것과도 맞는다.
- 1차 값 165 는 아크 상단(150)에서 15 밖에 안 떨어져 **엄지를 최대로 폈을 때만** 닿았다. "더 아래로"라는 제보와 정확히 맞물린다.

**[3] 경계 검사 (헤드리스 실측: 16:9 1280×720 / 20:9 915×412 / 21:9 1120×480 전부 동일)**

| 조건 | 값 | 여유 | 판정 |
|---|---|---|---|
| 링 사각 | x 97~193 / y **162~258** | — | 세 화면비에서 동일(좌·하단 앵커) |
| 링이 화면 안 | 아래끝 258 ≤ 360 | **102** | ✅ |
| 링이 아크 안 | 162~258 ⊂ 150~300 | 위 12 / 아래 42 | ✅ (165 일 때는 위끝 117 이 아크 밖으로 33 나갔다) |
| EXP 바 y 0~10 / HP 바 y 10~26 | 링 위끝 162 | **136** | ✅ |
| 인간성 심장 y **38~62** (실측) | 링 위끝 162 | **100** | ✅ |
| 녹턴 초상 y 66~131 (PACT 전용) | 링 위끝 162 | 31 | ✅ |
| 중앙 시야 금지 구역 x ≥ 200 | 링 우끝 193 | 7 | ✅ (x 는 안 건드렸다) |
| 대시 버튼 중심 (556, 292) | 중심거리 419 / 반경합 80+36=116 | 303 | ✅ 화면이 넓어지면 멀어지기만 한다 |

**[4] 잡기 반경 80 이 화면 밖으로 나가는가 → 안 나간다 (0 px)**

잡기 원은 `x 65~225 / y 130~290`. 좌측 65 / 아래 70 이 남아 **실효 잡기 면적이 잘리지 않는다**.
이게 y 를 상한 252 까지 더 내리지 않은 이유이기도 하다 — `y > 280` 이면 잡기 원 아래쪽이 화면 밖으로
나가 **보이는 곳 ≠ 먹히는 곳**이 된다(고정 모드의 존재 이유를 스스로 깬다).

**[5] 실드래그 검증 (CDP 실제 포인터, 16:9 1280×720)**

| 시작 지점(논리) | 축 거리 | `input.active` | 플레이어 이동 |
|---|---|---|---|
| (145, 210) 새 원점 | 0 | ✅ true | +63px |
| (145, 165) 1차 값 | 45 | ✅ true | 이동함 (반경 80 안) |
| (120, 200) 0차 값 | 27 | ✅ true | 이동함 (반경 80 안) |
| (145, 300) / (145, 120) | 90 | ❌ false | 0 |
| (60, 320) / (300, 120) | 밖 | ❌ false | 0 |
